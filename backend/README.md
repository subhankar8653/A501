...
- Self-update is **off by default** — Restart just restarts.
- If you maintain your own fork and want the Restart button to also pull
  it, set these environment variables (Railway → Variables, or
  `config.env` for other hosts) and set **Upstream Repo / Branch** in
  Settings to point at *your own* repo:
  - `ENABLE_SELF_UPDATE=true`
  - `UPSTREAM_REPO=https://github.com/<your-username>/<your-repo>`
  - `UPSTREAM_BRANCH=main` (or whichever branch you deploy from)
- With `ENABLE_SELF_UPDATE` unset (or `false`), Restart never touches git
  and never pulls code from anywhere but your own deployment.
...
| **Upstream Repo / Branch** | Only used if `ENABLE_SELF_UPDATE=true` is set. Point it at your own fork's repo/branch — leave blank to keep self-update off. |
...
This repo already includes a `heroku.yml` — connect the repo to a Heroku
app (Dashboard → Deploy → GitHub) and set the same variables from
`sample_config.env` in **Settings → Config Vars**.
...
### 🔑 Step 1: Create a Hugging Face Write Token
...
### 🔐 Step 3: Add Deploy Credentials to Your GitHub Repo
In **your repo** → **Settings → Secrets and variables → Actions**:

| Type | Name | Value |
| ------------ | ------------- | -------------------------------------- |
| **Secret** | `HF_TOKEN` | the Write token from Step 1 |
...
### 🤖 Step 4: Add Your Bot Secrets to the Space
...
### ▶️ Step 5: Deploy
In **your repo** → **Actions** → select **Deploy to Hugging Face Space** → **Run workflow**. After this first run, **every push auto-deploys**. Watch the build on your Space page — once it shows **Running**, you're live.

### 🎬 Step 6: Use Your Addon
...
Built on the [Stremio SDK](https://github.com/Stremio).
