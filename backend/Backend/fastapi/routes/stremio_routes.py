...
#----- Addon configuration
ADDON_NAME = "SuhaniBots"
ADDON_VERSION = __version__
PAGE_SIZE = 15

#----- Neutral placeholder used when an episode has no backdrop yet.
#----- Inline SVG data URI so it doesn't depend on any third-party host.
NO_EPISODE_BACKDROP = (
    "data:image/svg+xml;utf8,"
    "<svg xmlns='http://www.w3.org/2000/svg' width='300' height='169'>"
    "<rect width='100%' height='100%' fill='%23222'/>"
    "<text x='50%' y='50%' font-family='sans-serif' font-size='14' fill='%23888' "
    ...
    try:
        if id.startswith("custom_"):
            ...
            if id.startswith("anime_"):
                anime_filter = {"$or": [
                    {"is_anime": True},
                    {"$and": [{"original_language": "ja"}, {"genres": "Animation"}]},
                ]}
        ...
        for season in sorted(media.get("seasons", []), key=lambda s: s.get("season_number")):
            for episode in sorted(season.get("episodes", []), key=lambda e: e.get("episode_number")):
                ...
                videos.append({
                    ...
                    "thumbnail": _abs_media_url(episode.get("episode_backdrop")) or NO_EPISODE_BACKDROP,
