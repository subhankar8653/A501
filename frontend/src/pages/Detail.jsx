...
export default function Detail() {
  ...
  // ⋮ menu open on a single episode row (bottom sheet, dekho neeche), aur
  // download sheet ka target (either one episode, or the whole current season).
  ...
  // ROOT CAUSE FIX (user report: "Download/Save/Share options click nahi ho
  // rahe"): pehle yeh ek chhota dropdown tha jo har episode row ke ANDAR hi
  // absolute-positioned tha — us row par `active:scale-[0.98]` transform bhi
  // laga hua tha, jo dropdown ke andar tap karte hi (transform ek descendant
  // ke press se bhi trigger hota hai) turant lag jaata, box ko 2% chhota kar
  // deta, aur touch coordinates shift ho jaate — isi wajah se tap ka target
  // "hil" jaata aur click register hi nahi hota tha (ek jaana-maana mobile
  // hit-testing issue jab kisi transform wale ancestor ke andar absolute-
  // positioned button ho). Fix: menu ko row ke bahar nikaal ke, poore page
  // ke top-level par ek fixed bottom-sheet bana diya — bilkul season-picker
  // (`seasonPickerOpen`) jaisa hi pattern, jo pehle se reliably kaam kar
  // raha hai.
  const epMenuEpisode = useMemo(
    () => (epMenuOpen ? episodes.find((e) => e.id === epMenuOpen) || null : null),
    [epMenuOpen, episodes]
  )
  ...
  return (
    <div>
      ...
      {epMenuEpisode ? (
        <div className="fixed inset-0 z-[95] flex items-end justify-center" onClick={closeEpMenu}>
          <div className="absolute inset-0 bg-black/60" />
          <div
            onClick={(e) => e.stopPropagation()}
          ...
          >
            ...
            <button
              ...
              className="w-full flex items-center gap-3 py-3 text-sm font-medium text-reel-ink active:opacity-70 transition"
            >
              <span className="w-9 h-9 rounded-full bg-reel-gold/15 text-reel-gold flex items-center justify-center shrink-0">
            ...
            <button
              ...
              className="w-full flex items-center gap-3 py-3 text-sm font-medium text-reel-ink active:opacity-70 transition"
            >
              <span className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${saved ? 'bg-reel-gold text-reel-bg' : 'bg-reel-gold/15 text-reel-gold'}`}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill={saved ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/></svg>
              </span>
            ...
            <button
              onClick={() => {
                const ep = epMenuEpisode
                closeEpMenu()
                navigate(`/watch/${type}/${encodeURIComponent(ep.id)}`)
              }}
              className="w-full flex items-center gap-3 py-3 text-sm font-medium text-reel-ink active:opacity-70 transition"
            >
              <span className="w-9 h-9 rounded-full bg-reel-gold/15 text-reel-gold flex items-center justify-center shrink-0">
